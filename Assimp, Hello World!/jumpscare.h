...
public:
    ...
    void Trigger(glm::vec3 pos) {
        if (currentState == State::INACTIVE) {
            ...
            position.y = pos.y - 0.5f; // Leggermente pi� in basso per essere pi� minacciosa
            currentState = State::APPEARING;
            stateTimer.Start(0.3f); // Appare pi� velocemente per effetto shock
    ...
    void Update(float deltaTime) {
        ...
        if (!stateTimer.IsActive()) {
            if (currentState == State::APPEARING) {
                ...
                stateTimer.Start(0.5f); // Rimane visibile pi� a lungo
                std::cout << "JumpScare now ACTIVE" << std::endl;
            }
            else if (currentState == State::ACTIVE) {
                currentState = State::DISAPPEARING;
                stateTimer.Start(0.8f); // Scompare pi� lentamente
    ...
    void Draw(Shader& shader, const Camera& camera, const glm::mat4& projection, const glm::mat4& view) {
        ...
        // Calcola l'alpha in modo pi� semplice e visibile
        float alpha = 1.0f;
        if (currentState == State::APPEARING) {
        ...
        // Scala pi� grande per renderla pi� visibile
